async function handleBotReply(message) {
  try {
    ...
    // 📚 --- Show Book List ---
    if (msg.includes("books") || msg.includes("book list") || msg.includes("show books")) {
    ...
    if (msg.includes("paytm") && !(msg.includes("send") || msg.includes("money") || msg.includes("recharge") || msg.includes("use"))) {
      ...
      const paytmFaq = faqs.find(f => f.question.toLowerCase().includes("paytm"));
    ...
    // 🤖 --- Default fallback ---
    addMessage(
      "🤖 I can help with WhatsApp, Paytm, Google Maps, and Book List.\nTry asking:\n• How to send money using Paytm?\n• WhatsApp photo tutorial\n• Use Google Maps\n• Show me book list"
    );
